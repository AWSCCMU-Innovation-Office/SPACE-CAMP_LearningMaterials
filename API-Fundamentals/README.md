...
1. Open Git Bash and select (`cd {folder-name/directory-address}`) or create (`mkdir {folder-name}`) a folder you want to work this project on. Make sure you are in your desired working directory (e.g. `API-Fundamentals/` or `C:\User\Projects\API-Fundamentals`).
...
4. Start a `New Request` under API Fundamentals. Name it "Test Connection" and its method should be `GET`. If prompted to choose mode, choose `Developer Mode`.
...
2. Run the code using `fastapi dev main.py`.
3. Save and send the `Test Connection` Bruno request to check the output.
...
3. If prompted to enter a commit message, enter `Ctrl + C`, then type `:qa` and enter.
4. Run the code by entering `fastapi dev main.py`.
5. Create a Bruno request. Name it "Get Specific Book" and its method should be `GET`. Use the endpoint `/books/{book_id}` in the same base URL.
6. Modify `{book_id}` into `2`. Save and send the request. Observe the output.
...
3. Go back to VS Code and modify the code by moving up lines 28-31 right before line 21. Save the code and wait for the API to run again.

![image](https://github.com/user-attachments/assets/c42d5b3c-eda7-4d2c-ae7a-9d4e1783c060)

4. Send the request again. Verify and screenshot the output of the Bruno request showing the `Params` tab with the modified working code.
...
2. If prompted to enter a commit message, enter `Ctrl + C`, then type `:qa` and enter.
3. Run the code by entering `fastapi dev main.py`.
4. Create or clone a Bruno Request. Change its name to "Store a Book", then change its method to `POST` and endpoint to `/books` using the same base URL.
5. Go to `Body`, add a `JSON` Body by changing `No Body`, and enter a JSON formatted entry with title and author for the book you want to create. Save and send the request.
   
![part5](https://github.com/user-attachments/assets/c885ed17-c865-4f09-80c5-4f8b8bca5359)

5. Verify and screenshot the output of the request and if the book is added in the `Get All Books` Bruno request (resend the request) and the SQLite explorer table (click the play button to refresh the table). 

## Part 6: Create a PUT Endpoint for books

1. Use `Ctrl + C`, enter the command `git stash`, and then pull the Part 6 files by entering `git pull origin part6`. If prompted to enter a commit message, enter `Ctrl + C`, then type `:qa` and enter.
...
5. Change `{book-id}` endpoint to a valid book id you want to update. Save and send the request.

![part6](https://github.com/user-attachments/assets/c90ee9ba-4005-4a49-9546-3f5ac8aaed5f)

6. Verify and screenshot the output of the request if the book is updated in the `Get All Books` Bruno request and the SQLite explorer table. 

## Part 7: Create a DELETE endpoint for books

1. Use `Ctrl + C`, enter the command `git stash`, and then pull the Part 7 files by entering `git pull origin part7`. If prompted to enter a commit message, enter `Ctrl + C`, then type `:qa` and enter.
2. Run the code by entering `fastapi dev main.py`. 
...
6. Verify and screenshot the output of the request and if the book is deleted in the `Get All Books` Bruno request and the SQLite explorer table. 

## Part 8: API Authentication

1. Use `Ctrl + C`, enter the command `git stash`, and then pull the Part 8 files by entering `git pull origin part8`. If prompted to enter a commit message, enter `Ctrl + C`, then type `:qa` and enter.
...
6. Change the `Auth` of the `Get All Books` request from `No Auth` to `Inherit`.
...
- Mark Achilles Flores Jr. (MachiBytes)

### Learning Materials Prepared by: 
